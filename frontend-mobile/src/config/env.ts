 ...
 *
 * The local API is used whenever the app was loaded from a dev server, including `npm run start:fast`
 * (production-mode JavaScript, so __DEV__ is false). Only a real release build uses the production API.
 */
...
export const env = {
  apiBaseUrl: __DEV__ || METRO_HOST ? DEV_API_BASE_URL : PROD_API_BASE_URL,
